...
// Frames computed simultaneously
...
// Enables validation layers if compiled in debug mode
#ifdef NDEBUG
...
// Indices to represent the index buffer content
...
private:
	...
	void initWindow() {
		...
		// Creates a window without an OpenGL instance
	...
	void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
		...
		// Begin allocating memory for the buffer.
	...
	// Copies the contents from one buffer to another buffer.
	void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize
		size) {
		// A command buffer is allocated for the buffer copy operation.
		...
		// Creates the VkBufferCopy struct and begins copying the source
	...
	void createVertexBuffer() {
		// Calculates the required buffer size of the vertex data,
		// then calls createBuffer() to create the buffer.
		VkDeviceSize bufferSize = sizeof(vertices[0]) * vertices.size();
		VkBuffer stagingBuffer;
		VkDeviceMemory stagingBufferMemory;

		// Creates a buffer that is host visible. This is used to
		// transfer to a buffer that is local to the device.
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
			VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer,
			stagingBufferMemory);

		// The allocated memory is mapped to be accessible by the host.
		void* data;
	...
	// Defines the details for descriptor binding used in the shaders for pipeline creation.
	void createDescriptorSetLayout() {
		// Creates the required struct and assigns attributes.
	...
	void createGraphicsPipeline() {
		...
		// Enables color blending using alpha factors.
		...
		// Enables bitwise color blending. Enabling this disables alpha color blending!
		...
		// Begin references to previously created structures (fixed-fucntion)
	...
	// Detects vulkan-supported physical devices and inserts physical device
	...
	void pickPhysicalDevice() {
		...
		// Iterates through devices for one that supports queue families.
	...
	void createLogicalDevice() {
		...
		for (uint32_t ququqFamily : uniqueQueueFamilies) {
	...
	bool isDeviceSuitable(VkPhysicalDevice device) {
		...
		// The swap chain support is queried if the device supports extensions.
		...
		// The swap chain is queried only after the extension is verified
	...
	// Gets the device's queue family properties and stores the handles into a vector.
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		...
		// Counts the amount of queue flags. 
		// Requires that at least one queue that supports graphics operations.
		int i = 0;
	...
	// Queries details of the swap chain support. Checks for
	...
	SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device) {
		...
		// Queries for basic surface capabilities and are returned into the details struct.
		vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, surface, &details.capabilities);

		// The supported formats are queried and stored.
		uint32_t formatCount;
		vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &formatCount, nullptr);

		// The vector is resized to the format count size and stores the format into the details struct.
		if (formatCount != 0) {
			details.formats.resize(formatCount);
			vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &formatCount, details.formats.data());
		}

		// Queries for presentation modes and results are returned into the details struct.
		uint32_t presentModeCount;
		vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &presentModeCount, nullptr);

		// The vector is resized to the amount of presentation modes and stores the returned list into the
		// details struct.
	...
	void updateUniformBuffer(uint32_t currentImage) {
		// The chrono library is used to perform timekeeping independent of the frame rate.
		...
		// Rotates the model around the Z-axis at a rate of 90 degrees per second. The view is
		// from above at a 45 degree angle using a perspective projection with a vertical 45 
		...
		// Inverts the Y-coordinate of the projection matrix. The GLM library was designed for 
		// OpenGL, where the Y-coordinate of the of the clip coordinates is inverted. This
		// change prevents the image from rendering upside-down.
		ubo.proj[1][1] *= -1;

		// The data is directly copied to the current uniform buffer without the need of a
		// staging buffer.
		memcpy(uniformBuffersMapped[currentImage], &ubo, sizeof(ubo));
	}

	// Creates descriptor pools in which descriptor sets are allocated from.
	...
	// Enables debug messages if the debug flag is set
	...
	// Reports the message type and severity to the debug messanger
	...
	void createInstance() {
		...
		// Structure for application info
		...
		// Instance info, accepts application info structure
		...
		// Adds extension attributes to instance info
